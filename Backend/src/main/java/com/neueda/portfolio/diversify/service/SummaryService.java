...
public class SummaryService {
    ...
    public SummaryDTO getSummary() {
        try {
            ...
            int safeLimit = Math.max(1, recentInvestmentsCount);
            List<History> recentTxns = transactionHistoryRepo.findRecent(safeLimit);
            List<RecentInvestmentDTO> recent = recentTxns.stream()
                    .map(t -> new RecentInvestmentDTO(
                            t.getInvestmentOption().getName(),
                            normalizeCategory(t.getInvestmentOption().getCategory()),
                            t.getAction(),
                            t.getQuantity(),
                            t.getBoughtPrice(),
                            t.getPurchaseDate()
            ...
            throw new DiversifyServiceException("Failed to generate portfolio summary.", ex);
        }
    }
